...
def main(conf):
    ...
    ex_save_dir = os.path.join(conf['exp_dir'], 'examples_stage{}'.format(conf['stage'].replace(':','')))
    if conf['n_save_ex'] == -1:
    ...
    for idx in pbar:
        ...
        if(conf['stage'][0]==1 and conf['stage'][1]==1):
            est_sources, sources = model(mix[None,], sources[None,],do_test=True,stage='1:1')
        elif(conf['stage'][0]==1 and conf['stage'][1]==2):
            _, est_sources, _, sources = model(mix[None,], sources[None,],do_test=True,stage='1:2')
        elif(conf['stage'][0]>1 and conf['stage'][1]==1):
            _ , _, est_sources, sources = model(mix[None,], sources[None,],do_test=True,stage='{}:2'.format(conf['stage'][0]-1))
        elif(conf['stage'][0]>1 and conf['stage'][1]==2):
            _, est_sources, _, sources = model(mix[None,], sources[None,],do_test=True,stage='{}:2'.format(conf['stage'][0]))
        else:
            raise ValueError()
        sources = sources[:,:,0,:]
